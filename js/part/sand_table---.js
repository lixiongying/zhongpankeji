	...
	var vn=new Vue({
		...
		methods:{
			init_three:function(stoneType){
				var shef=this;
				$.ajax({    
				   url:'http://120.79.70.221:9281/globalstone/app/public/mallStore/appPortName.do',
				   type : 'post',
			...
			num_in:function(){
				var shef=this;
				$.ajax({    
				   url:'http://120.79.70.221:9281/globalstone/app/public/mallStore/appPortType.do',
				   type : 'post',
				   data:{}, 
				   dataType:"jsonp",    
				   jsonp:"callback",   
			...
			num_check:function(text){
				var shef=this;
				$.ajax({    
				   url:'http://120.79.70.221:9281/globalstone/app/public/mallStore/appPort.do',
				   type : 'post',
				   data:{sys_name:text}, 
				   dataType:"jsonp",    
				   jsonp:"callback",   
				   ...
				   success:function(json,textStatus){
				   	console.log(json)
				   		information = {
					   			name:json.mastoName,
					   			type:'',
					   			phone:json.mstoPhone,
					   			address:json.mstoAddress,
					   			main:json.mgoname,
						    }
						map_in(text,information);
						console.log(text)
				   },     
				   error:function(XMLHttpRequest,textStatus,errorThrown){  
				   	  console.log('错误'); 
...
//	              animation: qq.maps.MarkerAnimation.BOUNCE,//标记的动画
				...
				var anchor = new qq.maps.Point(0, 39),
				      ...
				      "./img/sand_table/coordinate1.png",
				      size, 
				      origin,
				      anchor
			    	);
...
//	    console.log(Arr[1]);
//		console.log(name.mstoName);
	    for(var i=0;i<Arr.length;i++){
	    	(function(){
	    	var name = Arr[i];
	    	var geo = new qq.maps.Geocoder()
		    geo.getLocation('中国,广州,'+name+'');
		    	geo.setComplete(function(res){
...
//	    	console.log(information);
    	...
    	map.panTo(new qq.maps.LatLng(23.1200491,113.30764968));
		...
		methods:{
			...
			init_one_on:function(json,shef_one){
				shef_one.name = json.mstoName;
		   		...
		   		map_in(shef_one.name,information);
			},
		}//methods函数结尾
	});
	}
			...
			methods:{
				init_two:function(){
					...
					$.ajax({    
					   ...
					   success:function(json,textStatus){
//					   		console.log(json.mstoName.length)
							init(json);
					   },     
					   error:function(XMLHttpRequest,textStatus,errorThrown){  
					   	  console.log('错误'); 
