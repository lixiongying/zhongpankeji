...
function uploadimge() {
	...
	var task = plus.uploader.createUpload(link_All.link_Ali + "/globalstone/app/private/mall/uploadPictures.do?museId=" + museId + "&musePwd=" + musePwd + "&museOnlineTag=" + museOnlineTag + "", {
		method: "POST"
	}, function(t, status) {
		if(status == 200) {
			plus.storage.clear();
			...
			if(data == ' ' || data == undefined || data == null) {
				...
				var link = link_All.link_Ali + 'globalstone/common/file/download.do?storeFileName='
				museImage = data.msgUrl;
				$("#userPhoto").attr('src', link + data.msgUrl)
			}
	...
	task.addData("id", localStorage.getItem('museId'));
...
function showImgDetail(imgId, imgkey, id, src) {
	...
	html += '<span class="del_icon" onclick="delImg(\'' + imgId + '\',\'' + imgkey + '\',' + id + ');"><img src="./img/information/shangpinxiangqing_quxiao.png" alt="" /></span>';
	html += '</div>';
	...
	plus.gallery.pick(function(e) {
		...
		function file() {
			plus.io.resolveLocalFileSystemURL(e.files[zm], function(entry) {
				compressImages(entry.toLocalURL(), entry.name, divid);
			}, function(e) {
				plus.nativeUI.toast("读取拍照文件错误：" + e.message);
			});
			zm++;
...
//压缩图片  
function compressImages(url, filename, divid) {
	var name = "_doc/upload/" + divid + "-" + filename; //_doc/upload/F_ZDDZZ-1467602809090.jpg  
	plus.zip.compressImage({
			src: url, //src: (String 类型 )压缩转换原始图片的路径  
			dst: name, //压缩转换目标图片的路径  
		...
		function(event) {
			...
			//event.target获取压缩转换后的图片url路  
			//filename图片名称  
			saveimages(event.target, divid, filename, path);
		},
		function(error) {
			plus.nativeUI.toast("压缩图片失败，请稍候再试");
		});
 ...
 * @param {Object} name   图片的名称  
 */
function saveimages(url, divid, name, path) {
	//alert(url);//file:///storage/emulated/0/Android/data/io.dcloud...../doc/upload/F_ZDDZZ-1467602809090.jpg  
	//alert(path);//_doc/upload/F_ZDDZZ-1467602809090.jpg  
	var state = 0;
	var wt = plus.nativeUI.showWaiting();
...
function uploadImgs() {
	...
	var numKeys = plus.storage.getLength();
	var parms = {};
	parms.contant = $('#texta').val();
	if(numKeys == 0 && parms.contant == '') {
		wa.close();
		return;
	}
	var task = plus.uploader.createUpload(link_All.link_Ali + "/globalstone/app/private/mall/uploadPictures.do?museId=" + museId + "&musePwd=" + musePwd + "&museOnlineTag=" + museOnlineTag + "", {
		...
		if(status == 200) {
			if(data == ' ' || data == undefined || data == null) {
				...
				mallStoreImages.push(data.msgUrl);
				//				$("#userPhoto").attr('src', link + data.msgUrl)
				//              alert(mallStoreImages)
			}
